            ...
            if request.method == 'POST':
                if form.is_valid():
                    if field_to_modify == 'dateCreated':
                        date_project = str(data_dataset['created_date']) + " 00:00:00+00:00" if data_dataset['created_date'] != 'None' else str(data_dataset['created_date'])
                        variables.sparql_generic_post_object.update_date_leaf(pid, field_to_modify,
                                                                    form.cleaned_data['created_date'],
                                                                    date_project)
                    elif field_to_modify == 'dateModified':
                        date_project = str(data_dataset['modified_date']) + " 00:00:00+00:00" if data_dataset['modified_date'] != 'None' else str(data_dataset['modified_date'])
                        variables.sparql_generic_post_object.update_date_leaf(pid, field_to_modify,
                                                                    form.cleaned_data['modified_date'],
                                                                    date_project)
